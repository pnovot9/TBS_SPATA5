...
the vault. There is no separate glossary source file: the vault's `> ℹ`
lines stay the single source of truth, and editing them updates both the
info popovers and the glossary. One `> ℹ` line can define several bold
terms. A bold span starts a new term only at the start of a sentence.
Bold mid-sentence stays inside the running definition. Duplicate terms
across articles merge into one entry that keeps the longest definition
and lists all source pages.

Term data is embedded in the page as JSON. Search, list rendering, and
term switching live in `app.js` and ignore diacritics, so "lecba" finds
"léčba". Chooser prototypes for this page live in
`web/designs/glossary-*.html`. Under 860 px the two panes stack.
